...
int main(void)
{
  ...
  // Set frequency scaling: S0=1, S1=0 → 20%
  ...
  /* USER CODE END 2 */
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_7, GPIO_PIN_RESET); // OE = LOW
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  // ----- RED -----
	  TCS3200_SetColor(0,0);   // RED filter
	  uint32_t red = TCS3200_ReadFreq();

	  TCS3200_SetColor(0,1);   // BLUE filter
	  uint32_t blue = TCS3200_ReadFreq();

	  TCS3200_SetColor(1,1);   // GREEN filter
	  uint32_t green = TCS3200_ReadFreq();

	  char msg[100];
	  sprintf(msg,"R=%lu  G=%lu  B=%lu\r\n",red,green,blue);
...
uint32_t TCS3200_ReadFreq(void)
{
    ...
    while ((HAL_GetTick() - start) < 100)   // 100ms gate time
    {
    ...
    return count * 10;   // convert 100ms count → Hz
}
